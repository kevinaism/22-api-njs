    ...
    if (!folders.length) {
        ...
        sftp.mkdir(FOLDER, function(dirErr) {
            if (dirErr) {
                ...
                telegram_bot.sendMessages(' JOOX: '+'folder - ' + FOLDER + ' has been created.', batchId);
    ...
    if (!files.length) {
        ...
        sftp.fastPut(FILE.local, FILE.remote, function(fileErr) {
            if (fileErr) {
                ...
                telegram_bot.sendMessages(' JOOX: '+'file - ' + FILE.remote + ' has been uploaded.', batchId);
...
module.exports = function(platform, dir, batchId, upc, xmlWrapper, files) {
    return new promise((resolve, reject) => {
        telegram_bot.sendMessages(' JOOX: '+'start upload', batchId);
        // create platform dir
                        ...
                        CreateFolders(sftp, [
                            ...
                            if (creatingDirsErr) {
                                ...
                                UploadFiles(sftp, files, batchId, () => {
                                    ...
                                    telegram_bot.sendMessages(' JOOX: '+'Done.', batchId);
                                    // all done
                                    resolve();
                                });
                            }
